...
namespace ColourClock
{
    public partial class Settings : Form
    {
        ...
        private void PhraseSettings()
        {
            for (var i = 0; i < 4; i++)
            {
                var txtBox = (TextBox)GetType().GetField("textBoxX" + (i + 1).ToString(), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).GetValue(this);
                int.TryParse(txtBox.Text, out _tempTransfer);
                _mainWindow.Xy[i].X = _tempTransfer;
                txtBox = (TextBox)GetType().GetField("textBoxY" + (i + 1).ToString(), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).GetValue(this);
                int.TryParse(txtBox.Text, out _tempTransfer);
                _mainWindow.Xy[i].Y = _tempTransfer;
                txtBox = (TextBox)GetType().GetField("textBoxR" + (i + 1).ToString(), System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic).GetValue(this);
                int.TryParse(txtBox.Text, out _tempTransfer);
